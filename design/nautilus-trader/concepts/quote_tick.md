...
### 6. Strategies Use QuoteTick

Strategies subscribe to QuoteTicks and implement `on_quote_tick()` to make trading decisions.

**Data Flow:** `MessageBus` → `Strategy.on_quote_tick()` → `Indicator.handle_quote_tick()` / `Order Management`

**Example 1: Market Making Strategy**

| Step | Action | QuoteTick Data Used |
|------|--------|---------------------|
| 1. Subscribe | `strategy.subscribe_quote_ticks(instrument_id)` | - |
| 2. Receive QuoteTick | `on_quote_tick(quote: QuoteTick)` | bid_price: 10000.0000<br>ask_price: 10001.0000 |
| 3. Calculate Order Prices | `bid_price - offset` = 9999.0000<br>`ask_price + offset` = 10002.0000 | bid_price, ask_price |
| 4. Submit Orders | Submit limit orders at calculated prices | - |
...
|------|--------|---------------------|
| 1. Subscribe | `strategy.subscribe_quote_ticks(instrument_id)` | - |
| 2. Receive QuoteTick | `on_quote_tick(tick: QuoteTick)` | bid_price: 1.0500<br>ask_price: 1.0505 |
| 3. Update Indicator | `macd.handle_quote_tick(tick)` | Extracts mid price: 1.05025 |
| 4. Check Signal | `if macd.initialized: check_signals()` | Indicator value from QuoteTick |

### 7. Indicators Use QuoteTick

Indicators implement `handle_quote_tick()` to update their calculations using bid, ask, or mid prices.

**Data Flow:** `Strategy.on_quote_tick()` → `Indicator.handle_quote_tick()` → `Indicator.update()`

**Example: SpreadAnalyzer**

| QuoteTick Input | Indicator Processing | Output |
|-----------------|----------------------|--------|
| bid_price: 1.121200<br>ask_price: 1.121720 | Calculate spread: `ask - bid` = 0.000520<br>Store in deque (capacity: 100)<br>Update average spread | current: 0.000520<br>average: 0.000485 |

**Example: MACD Indicator**

| QuoteTick Input | Indicator Processing | Output |
|-----------------|----------------------|--------|
| bid_price: 1.0500<br>ask_price: 1.0505 | Extract mid price: `(bid + ask) / 2` = 1.05025<br>Update MACD calculation<br>Check if initialized | macd_value: 0.0012<br>signal: 0.0008<br>histogram: 0.0004 |
...
4. Strategies receive QuoteTick via `on_quote_tick()` callback
5. Indicators receive QuoteTick via `handle_quote_tick()` method
