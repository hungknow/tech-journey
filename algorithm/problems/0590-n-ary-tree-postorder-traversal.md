# 0590 N-ary Tree Postorder Traversal

## Problem Description

Given the `root` of an n-ary tree, return the postorder traversal of its nodes' values.

N-ary tree input serialization is represented in their level order traversal, and each group of children is separated by the null value.

### Example 1:
```
Input: root = [1,null,3,2,4,null,5,6]
Output: [5,6,3,2,4,1]
```

### Example 2:
```
Input: root = [1,null,2,3,4,5,null,null,6,7,null,8,null,9,10,null,null,11,null,12,null,13,null,null,14]
Output: [2,6,14,11,7,3,12,8,4,13,9,10,5,1]
```

## The Twist

N-ary trees have **multiple children** per node. Postorder visits all children first (left to right), then the root.

## Algorithm

### Recursive Approach:
1. If root is null, return empty list
2. For each child in order: recursively traverse child
3. Add root value to result

### Iterative Approach (using two stacks):
1. Push root onto stack1
2. While stack1 is not empty:
   - Pop node from stack1, push to stack2
   - Push all children of node to stack1 (in order)
3. Pop all nodes from stack2 to get postorder

## Complexity

- **Time**: O(n) - each node visited once
- **Space**: O(h) for recursive, O(n) for iterative stack

## Link

[LeetCode 0590 N-ary Tree Postorder Traversal](https://leetcode.com/problems/n-ary-tree-postorder-traversal/)
