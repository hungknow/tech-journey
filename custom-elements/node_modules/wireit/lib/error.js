...
${drawSquiggle(diagnostic.location, 4)}`;
    ...
    _printSupplemental(supplemental) {
    ...
    _formatPath(location) {
        const relPath = pathlib.relative(this._cwd, location.file.path);
        const { line, character } = this._offsetToPosition(location.file, location.range.offset);
        return `${CYAN}${relPath}${RESET}:${YELLOW}${line}${RESET}:${YELLOW}${character}${RESET}`;
    }
    _offsetToPosition(file, offset) {
...
OffsetToPositionConverter._cache = new WeakMap();
const CYAN = '\x1b[36m';
