...
export class Analyzer {
    constructor(agent, filesystem) {
        this._placeholders = new Map();
        this._ongoingWorkPromises = [];
        this._relevantConfigFilePaths = new Set();
        this._agent = agent;
    ...
    async analyzeFiles(files) {
        ...
        for (const info of this._placeholders.values()) {
            ...
            this._checkForCyclesAndSortDependencies(info.placeholder, new Set(), true);
        }
    ...
    async analyze(root, extraArgs) {
        ...
        const cycleResult = this._checkForCyclesAndSortDependencies(rootConfig, new Set(), true);
        if (!cycleResult.ok) {
            return {
    ...
    async _waitForAnalysisToComplete() {
        while (this._ongoingWorkPromises.length > 0) {
            const promise = this._ongoingWorkPromises[this._ongoingWorkPromises.length - 1];
            await promise;
     ...
     */
    async _upgradePlaceholder(placeholder) {
        ...
        if (syntaxInfo === undefined || syntaxInfo.scriptNode === undefined) {
            ...
            if (syntaxInfo?.wireitConfigNode?.name !== undefined) {
                ...
                node ?? (node = packageJson.scriptsSection?.name);
        ...
        const { dependencies, encounteredError: dependenciesErrored } = this._processDependencies(placeholder, packageJson, syntaxInfo);
        let command;
        let commandError = false;
        if (wireitConfig === undefined) {
            const result = failUnlessNonBlankString(scriptCommand, packageJson.jsonFile);
            if (result.ok) {
        ...
        const allowUsuallyExcludedPaths = this._processAllowUsuallyExcludedPaths(placeholder, packageJson, syntaxInfo);
        const files = this._processFiles(placeholder, packageJson, syntaxInfo, allowUsuallyExcludedPaths);
        if (wireitConfig !== undefined &&
        ...
        const output = this._processOutput(placeholder, packageJson, syntaxInfo, command, allowUsuallyExcludedPaths);
        const clean = this._processClean(placeholder, packageJson, syntaxInfo);
        const service = this._processService(placeholder, packageJson, syntaxInfo, command, output);
        await this._processPackageLocks(placeholder, packageJson, syntaxInfo, files);
    ...
    _processDependencies(placeholder, packageJson, scriptInfo) {
        const dependencies = [];
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            const result = this._resolveDependency(unresolved, placeholder, packageJson.jsonFile);
            if (!result.ok) {
                encounteredError = true;
                placeholder.failures.push(result.error);
                continue;
            ...
            for (const resolved of result.value) {
                ...
                this._ongoingWorkPromises.push((async () => {
    ...
    _processAllowUsuallyExcludedPaths(placeholder, packageJson, syntaxInfo) {
    ...
    _processFiles(placeholder, packageJson, syntaxInfo, allowUsuallyExcludedPaths) {
    ...
    _processOutput(placeholder, packageJson, syntaxInfo, command, allowUsuallyExcludedPaths) {
        if (syntaxInfo.wireitConfigNode === undefined) {
            return;
        }
    ...
    _processClean(placeholder, packageJson, syntaxInfo) {
    ...
    _processService(placeholder, packageJson, syntaxInfo, command, output) {
        if (syntaxInfo.wireitConfigNode === undefined) {
            return undefined;
        }
    ...
    async _processPackageLocks(placeholder, packageJson, syntaxInfo, files) {
        if (syntaxInfo.wireitConfigNode === undefined) {
            return;
        }
        ...
        files !== undefined &&
            ...
            const lockfileNames = packageLocks?.values ?? DEFAULT_LOCKFILES[this._agent];
    ...
    _processEnv(placeholder, packageJson, syntaxInfo, command) {
        if (syntaxInfo.wireitConfigNode === undefined) {
            return {};
        }
     ...
     */
    _checkForCyclesAndSortDependencies(config, trail, isPersistent) {
        ...
        if (config.dependencies.length > 0) {
            ...
            for (const dependency of config.dependencies) {
                ...
                const validDependencyConfigResult = this._checkForCyclesAndSortDependencies(dependency.config, trail, 
                ...
                if (!validDependencyConfigResult.ok) {
                    return {
                        ok: false,
                        error: this._markAsInvalid(config, validDependencyConfigResult.error.dependencyFailure),
                    };
                }
     ...
     */
    _resolveDependency(dependency, context, referencingFile) {
        ...
        if (dependency.value.startsWith('.')) {
            ...
            const result = this._resolveCrossPackageDependency(dependency, context, referencingFile);
            if (!result.ok) {
                return result;
            }
     ...
     */
    _resolveCrossPackageDependency(dependency, context, referencingFile) {
