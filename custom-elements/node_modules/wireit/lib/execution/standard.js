...
export class StandardScriptExecution extends BaseExecutionWithCommand {
     ...
     */
    async _acquireSystemLockIfNeeded(workFn) {
     ...
     * `.wireit` directory.
     */
    async _fingerprintIsFresh(fingerprint) {
     ...
     */
    async _handleCacheHit(cacheHit, fingerprint) {
        ...
        if (outputFilesAfterRunning.value !== undefined) {
            await this._writeOutputManifest(await this._computeOutputManifest(outputFilesAfterRunning.value));
        }
     ...
     */
    async _handleNeedsRun(fingerprint) {
    ...
    async _shouldClean(fingerprint) {
        ...
        switch (cleanValue) {
            ...
            case 'if-file-deleted': {
                ...
                return this._anyInputFilesDeletedSinceLastRun(fingerprint, prevFingerprint);
            }
            default: {
     ...
     */
    _anyInputFilesDeletedSinceLastRun(curFingerprint, prevFingerprint) {
     ...
     */
    async _saveToCacheIfPossible(fingerprint) {
     ...
     */
    _globOutputFilesBeforeRunning() {
        this._ensureState('before-running');
        return (this._cachedOutputFilesBeforeRunning ?? (this._cachedOutputFilesBeforeRunning = this._globOutputFiles()));
    }
    /**
     * Glob the output files for this script and cache them, but throw unless the
     * script has finished running or been restored from cache.
     */
    _globOutputFilesAfterRunning() {
        this._ensureState('after-running');
        return (this._cachedOutputFilesAfterRunning ?? (this._cachedOutputFilesAfterRunning = this._globOutputFiles()));
    }
    /**
     * Glob the output files for this script, or return undefined if output files
     * are not defined.
     */
    async _globOutputFiles() {
     ...
     */
    async _readPreviousFingerprint() {
        if (this._cachedPreviousFingerprint === undefined) {
            this._cachedPreviousFingerprint = (async () => {
                try {
                    return Fingerprint.fromString((await fs.readFile(this._fingerprintFilePath, 'utf8')));
                }
                catch (error) {
     ...
     */
    async _writeFingerprintFile(fingerprint) {
     ...
     */
    async _computeOutputManifest(outputEntries) {
    ...
    async _outputManifestIsFresh() {
        ...
        const newManifest = await this._computeOutputManifest(outputFilesBeforeRunning.value);
     ...
     */
    async _readPreviousOutputManifest() {
        try {
            return (await fs.readFile(this._outputManifestFilePath, 'utf8'));
        }
        catch (error) {
            if (error.code === 'ENOENT') {
                return undefined;
     ...
     */
    async _writeOutputManifest(outputManifest) {
        await fs.mkdir(this._dataDir, { recursive: true });
