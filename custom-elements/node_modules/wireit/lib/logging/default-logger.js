        ...
        switch (type) {
            ...
            case 'failure': {
                ...
                switch (reason) {
                    ...
                    case 'launched-incorrectly': {
                        console.error(`❌${prefix} wireit must be launched with "npm run" or a compatible command.`);
                        console.error(`    More info: ${event.detail}`);
                        break;
                    }
                    case 'missing-package-json': {
                        console.error(`❌${prefix} No package.json was found in ${event.script.packageDir}`);
                        break;
                    }
                    ...
                    case 'no-scripts-in-package-json': {
                        console.error(`❌${prefix} No "scripts" section defined in package.json in ${event.script.packageDir}`);
                        break;
                    }
                    ...
                    case 'exit-non-zero': {
                        console.error(`❌${prefix} Failed with exit status ${event.status}`);
                        break;
                    }
                    case 'signal': {
                    ...
                    case 'unknown-error-thrown': {
                        console.error(`❌${prefix} Internal error! Please file a bug at https://github.com/google/wireit/issues/new, mention this message, that you encountered it in wireit version ${getWireitVersion()}, and give information about your package.json files.\n    Unknown error thrown: ${String(event.error)}`);
            ...
            case 'info': {
                ...
                switch (detail) {
                    ...
                    case 'running': {
                        console.log(`🏃${prefix} Running command "${event.script.command?.value ?? ''}"`);
                        break;
                    }
                    case 'locked': {
                        console.log(`💤${prefix} Waiting for another process which is already running this script.`);
                        break;
                    }
                    case 'output-modified': {
                        console.log(`ℹ️${prefix} Output files were modified since the previous run.`);
                        break;
                    }
